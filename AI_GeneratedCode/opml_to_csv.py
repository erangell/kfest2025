...
def process_outline_element(element, current_path=None, result=None):
    ...
    if element.tag == 'outline':
        ...
        if text and current_path:  # Skip root level with empty path
            # Create dotted numeric string
            dotted_numeric = '.'.join(map(str, current_path))
            # Convert to NLS format
            nls_format = convert_to_nls_format(dotted_numeric)
            # Add to results
            result.append((dotted_numeric, nls_format, text))
...
def opml_to_csv(opml_file, csv_file):
    ...
    try:
        ...
        with open(csv_file, 'w', newline='', encoding='utf-8') as f:
            writer = csv.writer(f)
            # Write header
            writer.writerow(["dotted_numeric", "dotted_alphanumeric", "text"])
            # Write data
            for row in results:
                writer.writerow(row)
